...
const LoginForm = ({ onClose, isModal = false, onLoginSuccess }) => {
  ...
  // Redirect if user is already logged in (only for route usage)
  useEffect(() => {
    if (user && !isModal) {
      const redirectTo = location.state?.from || '/dashboard';
      navigate(redirectTo, { replace: true });
    }
  }, [user, navigate, location.state, isModal]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // Handle redirect based on usage
        setTimeout(() => {
          if (isModal) {
            if (onLoginSuccess) {
              onLoginSuccess();
  ...
  // If used as a route, wrap in a container
  if (!isModal) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-purple-50 flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          {/* Login Form Content */}
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-300 ease-in-out">
            {/* Header */}
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Close Button - Only show for route usage */}
                <div className="text-center">
                  <button
                    type="button"
                    onClick={handleClose}
